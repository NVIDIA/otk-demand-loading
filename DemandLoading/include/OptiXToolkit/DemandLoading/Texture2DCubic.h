...
textureCubic( const DeviceContext& context, unsigned int textureId, float s, float t, float2 ddx, float2 ddy, 
              bool conservativeFilter, unsigned int filterMode,
              TYPE* result, TYPE* dresultds, TYPE* dresultdt, float2 texelJitter = float2{} )
{
    ...
    textureCubic( texture, texWidth, texHeight, filterMode, mipmapFilterMode, maxAnisotropy, conservativeFilter,
...
{
    return textureCubic( context, textureId, s, t, ddx, ddy, conservativeFilter, filterMode, result, nullptr, nullptr, texelJitter );
}
             ...
             TYPE* result, TYPE* dresultds, TYPE* dresultdt, float2 texelJitter = float2{} )
{
    ...
    return textureCubic( context, textureId, s, t, ddx, ddy, conservativeFilter, filterMode, result, dresultds, dresultdt, texelJitter );
}
...
textureUdim( const DeviceContext& context, unsigned int textureId, float s, float t, float2 ddx, float2 ddy,
             ...
             TYPE* result, float2 texelJitter = float2{} )
{
    return textureUdim<TYPE>( context, textureId, s, t, ddx, ddy, conservativeFilter, filterMode, result, nullptr, nullptr, texelJitter );
}
         ...
         int nchannels, float* result, float* dresultds, float* dresultdt, float2 texelJitter = float2{} )
{
    ...
    do
    {
        resident &= textureUdim<float4>( context, textureId + (idx>>2), s, t, ddx, ddy, conservativeFilter, filterMode, resPtr, drdsPtr, drdtPtr, texelJitter );
...
{
    return texture( context, textureId, s, t, ddx, ddy, conservativeFilter, filterMode, nchannels, result, nullptr, nullptr, texelJitter );
}
